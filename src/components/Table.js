...
const TableComponent = ({ data, handleDelete, handleUpdate, isLoading,role }) => {
        ...
        <Table stickyHeader aria-label="sticky table">
          <TableHead>
            <TableRow>
              ...
              <TableCell align="center" style={{ backgroundColor: theme.palette.primary.main, color: theme.palette.getContrastText(theme.palette.primary.main) }}>
                Actions
              </TableCell>
            </TableRow>
          </TableHead>
          <TableBody>
            {data ? (
                  ...
                  <TableRow key={row.complainId}>
                    {columnsToShow.map((column) => (
                     ...
                     {column.id == 'userId' && role == 'Admin'
                       ? usersData.find((user) => user.id == row.userId)?.email || row.userId
                       : row[column.id]}
                   </TableCell>
                    ))}
                    <TableCell align="center">
                      <IconButton aria-label="delete" onClick={() => handleDelete(row.complaintId,row.userId)}>
                        <DeleteIcon />
                      </IconButton>
              ...
              <TableRow>
                <TableCell colSpan={columns.length + 1} align="center">
                {isLoading ? <CircularProgress /> : <Typography variant="body1">No records found.</Typography>}
                </TableCell>
              </TableRow>
            )}
            {emptyRows > 0 && (
